<?php
class ControllerCatalogDiytype extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->setTitle('商品材质');
		// var_dump($this->request->get);die;
		if(isset($this->request->get['ajax']) && $this->request->get['ajax'] == 1) {
			$data = $this->model_catalog_diytype->getAttributes();
			$this->response->addHeader('Content-Type: application/json');
			foreach ($data as $key => $value) {
				$data[$key]['thumbs'] = $this->model_tool_image->resize( $value['thumb'], 100,100);
			}
			
			return $this->response->setOutput(json_encode([
				'code'=> 0,
				'msg'=> '',
				'count'=> $this->model_catalog_diytype->getTotalAttributes(),
				'data'=> $data
			]));
		}
		...
		$data['saveurl'] = $this->url->link('catalog/diytype/save', 'user_token=' . $this->session->data['user_token'], true);
		$data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');
		$this->response->setOutput($this->load->view('catalog/diytype_list', $data));
	...
	public function save(){
		...
		if($data['id'] >0) {
			$this->model_catalog_diytype->editAttribute($data['id'], $data);
		}else{
			$this->model_catalog_diytype->addAttribute($data);
		}
	...
	public function delete() {
		$this->load->model('catalog/diytype');
		$this->model_catalog_diytype->deleteAttribute($this->request->get['id']);
		return $this->response->setOutput(json_encode([
				'code'=> 0,
			]));
	}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/diytype', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('catalog/diytype/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/diytype/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$attribute_total = $this->model_catalog_diytype->getTotalAttributes();

		$results = $this->model_catalog_diytype->getAttributes($filter_data);
		...
		foreach ($results as $result) {
			$data['attributes'][] = array(
				...
				'edit'            => $this->url->link('catalog/diytype/edit', 'user_token=' . $this->session->data['user_token'] . '&attribute_id=' . $result['attribute_id'] . $url, true)
			);
		}
