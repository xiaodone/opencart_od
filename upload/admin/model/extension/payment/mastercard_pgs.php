...
class ModelExtensionPaymentMastercardPGS extends Model {
    ...
    public function hookEvents() {
        ...
        foreach ($events as $trigger => $action) {
            $this->model_setting_event->addEvent('payment_mastercard_pgs', $trigger, $action, 1, 0);
        }
    }
    ...
    public function unhookEvents() {
        ...
        $this->model_setting_event->deleteEventByCode('payment_mastercard_pgs');
    }

    public function getTotalTransactions($data) {
        $sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "mastercard_pgs_transaction`";
