...
class ControllerExtensionPaymentMastercardPGS extends Controller {
    public function index() {
        ...
        $data['onclick'] = $this->config->get('payment_mastercard_pgs_onclick');

        $data['error_session'] = '';
        $data['error_event_missing'] = $this->language->get('error_event_missing');

        if (stripos($this->config->get('payment_mastercard_pgs_merchant'), 'TEST') !== FALSE) {
    ...
    public function checkout() {
        ...
        if ($this->request->server['REQUEST_METHOD'] != 'POST' || empty($this->request->post['token']) || @base64_decode($this->request->post['token']) === FALSE) {
            ...
            if ($this->config->get('payment_mastercard_pgs_checkout') == 'pay') {
    ...
    public function success() {
        ...
        if (
            ...
            if ($this->config->get('payment_mastercard_pgs_tokenize') && empty($session_data['sourceOfFunds']['token'])) {
    ...
    public function callback() {
        ...
        if ($this->config->get('payment_mastercard_pgs_debug_log')) {
            ...
            switch ($risk_gateway_code) {
                case 'REJECTED' : {
                    $this->model_extension_payment_mastercard_pgs->addOrderHistory(array(
                        ...
                        'order_status_id' => (int)$this->config->get('payment_mastercard_pgs_risk_rejected_order_status_id'),
                        'message' => sprintf(
                ...
                case 'REVIEW_REQUIRED' : {
                    ...
                    if (!$decision_exists || $decision_skipped) {
                        ...
                        $this->model_extension_payment_mastercard_pgs->addOrderHistory(array(
                            'order_id' => (int)$transaction_info['order']['reference'],
                            'order_status_id' => (int)$this->config->get('payment_mastercard_pgs_risk_review_' . strtolower($transaction_info['risk']['response']['review']['decision']) . '_order_status_id'),
                            'message' => sprintf(
                                $this->language->get('text_callback'),
                                $transaction_info['transaction']['type'],
        ...
        if ($transaction_info['response']['gatewayCode'] == 'APPROVED') {
            switch ($transaction_info['transaction']['type']) {
                ...
                case 'AUTHORIZATION_UPDATE' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_authorization_order_status_id');
                } break;
                case 'CAPTURE' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_capture_order_status_id');
                } break;
                case 'PAYMENT' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_payment_order_status_id');
                } break;
                case 'REFUND_REQUEST' :
                case 'REFUND' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_refund_order_status_id');
                } break;
                case 'VOID_AUTHORIZATION' : 
                case 'VOID_CAPTURE' : 
                case 'VOID_PAYMENT' : 
                case 'VOID_REFUND' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_void_order_status_id');
                } break;
                case 'VERIFICATION' : {
                    $order_status_id = $this->config->get('payment_mastercard_pgs_approved_verification_order_status_id');
                } break;
            }
        } else {
            $order_status_id = $this->config->get('payment_mastercard_pgs_' . strtolower($transaction_info['response']['gatewayCode']) . '_order_status_id');
        }
    ...
    protected function validate_callback() {
        ...
        if (!$this->config->get('payment_mastercard_pgs_notification_secret') || $this->config->get('payment_mastercard_pgs_notification_secret') != $this->model_extension_payment_mastercard_pgs->getHeaderVar('HTTP_X_NOTIFICATION_SECRET')) {
    ...
    protected function configuration($hosted = true) {
        ...
        if ($hosted && $this->config->get('payment_mastercard_pgs_google_analytics_property_id')) {
            $c['interaction']['googleAnalytics']['propertyId'] = $this->config->get('payment_mastercard_pgs_google_analytics_property_id');
        }
        ...
        if ($hosted) {
            ...
            $c['merchant'] = $this->config->get('payment_mastercard_pgs_merchant');
