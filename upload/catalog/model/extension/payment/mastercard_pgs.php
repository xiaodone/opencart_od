...
class ModelExtensionPaymentMastercardPGS extends Model {
    ...
    private $extension_version = '3.0.4';
    ...
    public function getGateway() {
        if ($this->config->get('payment_mastercard_pgs_gateway') != 'other') {
            return 'https://' . $this->config->get('payment_mastercard_pgs_gateway') . '-gateway.mastercard.com';
        } else {
            $url_parts = parse_url($this->config->get('payment_mastercard_pgs_gateway_other'));
    ...
    public function getMethod($address, $total) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_mastercard_pgs_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if ($this->config->get('payment_mastercard_pgs_total') > 0 && $this->config->get('payment_mastercard_pgs_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('payment_mastercard_pgs_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $title_config = $this->config->get('payment_mastercard_pgs_display_name');
            ...
            $method_data = array(
                ...
                'terms'                            => '',
                'sort_order'                    => $this->config->get('payment_mastercard_pgs_sort_order')
            );
        }
    ...
    public function editOrderPaymentDetails($order_id, $data) {
        ...
        $payment_address_1 = !empty($data['street']) ? $data['street'] : '';
        $payment_address_2 = !empty($data['street2']) ? $data['street2'] : '';

        $payment_postcode = !empty($data['postcodeZip']) ? $data['postcodeZip'] : '';
    ...
    public function api($api_method, $data = array(), $method = 'GET') {
        ...
        $url = $gateway . $this->config->get('payment_mastercard_pgs_merchant') . '/' . $api_method;
        
        $userid = 'merchant.' . $this->config->get('payment_mastercard_pgs_merchant');
        $password = $this->config->get('payment_mastercard_pgs_integration_password');
